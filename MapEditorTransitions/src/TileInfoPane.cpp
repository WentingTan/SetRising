...
void TileInfoPane::init()
{
	...
	// Initialize the exit field
	nExit.setPosition(sf::Vector2f(835.0f, 345.0f));
	nExit.setTextAreaOutline(2, sf::Color::Black);
	...
	enemySpawn.setOption(std::string("Patrolling Snake"), 16, 1);
	enemySpawn.setOption(std::string("Stationary Snake"), 16, 2);
...
void TileInfoPane::getTileInfo(Tile *t, sf::Vector2i index)
{
	...
	// Set the exit text field
	ss.str("");
	ss << tile->exit;
	nExit.setEnteredText(ss.str());
...
void TileInfoPane::update(float dt)
{
	...
	if (apply.wasClicked())
	{
		...
		tile->exit = atoi(nExit.getEnteredText().c_str());
