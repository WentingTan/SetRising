...
void PSEDeathHandler::handleEvent(Event::Data e)
{
	if (e.type == Event::ENEMY_DEATH && e.enemyType == E_BOSS)
		pPS->pushWin();
}
...
void PlayState::pushWin()
{
	pGame->requestStateChange(GSCR_PUSH_WIN);
}
...
bool PlayState::init()
{
	...
	eDeathHandler = new PSEDeathHandler(this);
