...
// Returns true if enemy is offscreen by more than a full screen width/screen height
bool Enemy::isOffscreen()
{
	sf::Vector2f pos = sprite.getPosition();

	if (pos.x < -1.0f * SCREEN_WIDTH || pos.x > 2.0f * SCREEN_WIDTH)
		return true;
	if (pos.y < -1.0f * SCREEN_HEIGHT || pos.y > 2.0f * SCREEN_HEIGHT)
		return true;
	return false;
}
...
bool Enemy::isTouchingFloor(TileMap *tm)
{
	int minX, maxX, y;

	sf::Vector2f lCorner;
	sf::Vector2f rCorner;
	lCorner.x = hitbox.left;
	lCorner.y = hitbox.top + hitbox.height;
	rCorner.x = hitbox.left + hitbox.width;
	rCorner.y = lCorner.y;

	y = tm->getTileIndex(lCorner.x, lCorner.y).y;
	minX = tm->getTileIndex(lCorner.x, lCorner.y).x;
	maxX = tm->getTileIndex(rCorner.x, rCorner.y).x;

	for (int x = minX; x < maxX + 1; x++)
		if (tm->indexIsWithinBounds(x, y))
			if (tm->isTileObstacle(x, y))
				return true;
