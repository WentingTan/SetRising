...
float Boss::getNextShootTime(float low, float high)
{
...
void Boss::init()
{
	...
	*/
	...
	perFrameHBCoords[0].left = 74.0;
	perFrameHBCoords[0].top = 90.0;
	perFrameHBCoords[0].width = 80.0;
	perFrameHBCoords[0].height = 80.0;

	perFrameHBCoords[1].left = 74.0;
	perFrameHBCoords[1].top = 90.0;
	perFrameHBCoords[1].width = 80.0;
	perFrameHBCoords[1].height = 80.0;

	perFrameHBCoords[2].left = 74.0;
	perFrameHBCoords[2].top = 90.0;
	perFrameHBCoords[2].width = 80.0;
	perFrameHBCoords[2].height = 80.0;
...
bool Boss::update(float dt, sf::Vector2f pPos)
{
	...
	else
	{
		...
		if (shootTimer > nextShootTime)
		{
			...
			if (shots < BOSS_NUM_SHOTS_IN_BURST)
				nextShootTime = BOSS_BURST_TIME;
			else
			{
				nextShootTime = 1.5f;
				shots = 0;
...
void Boss::shoot()
{
	...
	if (dir > 0.0f)
		e.posX = bounds.left + 0.75f * bounds.width;
	else
		e.posX = bounds.left + 0.25f * bounds.width;
	e.posY = bounds.top + 0.35f * bounds.height;
	e.dir = dir;
