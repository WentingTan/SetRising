...
{
	if (e.type == Event::ENEMY_DEATH || e.type == Event::DESPAWN_ENEMY || e.type == Event::DESPAWN_WEAPON || e.type == Event::DESPAWN_BARRIER)
		pGW->getActiveMap()->resetSpawnPoint(e.tile, e.enemyType);
}
...
void GWContinueHandler::handleEvent(Event::Data e)
{
	if (e.type == Event::CONTINUE)
		pGW->transition(START_ENTRANCE);
}
...
void GameWorld::init()
{
	...
	continueHandler = new GWContinueHandler(this);

	EventManager::addHandler(Event::ENEMY_DEATH, eDeathHandler);
	EventManager::addHandler(Event::DESPAWN_ENEMY, eDeathHandler);
	EventManager::addHandler(Event::DESPAWN_WEAPON, eDeathHandler);
	EventManager::addHandler(Event::DESPAWN_BARRIER, eDeathHandler);
...
void GameWorld::transition(int ent)
{
	...
	log << "\nIn GameWorld::transition():\n";
	log << "  entrance = " << ent << std::endl;
	log << "  mapID = " << transitions[ent].tileMapID << std::endl;
	log << "  entry tile = " <<  transitions[ent].entryTile.x << ", " <<  transitions[ent].entryTile.y << std::endl;
	log << "  ref tile = " << transitions[ent].referenceTile.x << ", " << transitions[ent].referenceTile.y << std::endl;
	log << "  reposition type = " << transitions[ent].reposition << std::endl;
	log << "  entry position = " << entryPos.x << ", " << entryPos.y << std::endl;
	log.close();
