...
bool Player::init()
{
	...
	if (!texture.loadFromFile("mm.png"))
		return false;
...
bool Player::isOnGround()
{
	...
	for (int x = index.x - 1; x < index.x + 2; x++)
	{
		...
		if (map->getTileHitBox(x, y, box))
		{
			// Ensure that the collision overlap is at least a pixel wide
			// otherwise a collision with a wall might be mistaken for a 
...
bool Player::hasHitWall(float direction)
{
	...
	for (int y = index.y - 1; y < index.y + 2; y++)
	{
		...
		if (map->getTileHitBox(x, y, box))
		{
			// Ensure that the collision overlap is at least a 3 pixels high
			// otherwise a collision with the floor might be mistaken for a 
			// collision with a wall
			if (hitbox.intersects(box, overlap) && overlap.height > 3.0f)
			{
				// Move player so that it is no longer in collision with the wall
