...
void PlayerProjectiles::checkCollisions(BarrierManager *barriers)
{
	...
	while (lasers[i].isActive() && i < MAX_LASERS)
		//if (enemies->checkCollisions(&lasers[i]))
		if (barriers->checkCollisions(&lasers[i]))
            remove(i, W_LASER);
		else
            i++;
	...
	while (freezeRays[i].isActive() && i < MAX_FREEZE_RAYS)
		//if (enemies->checkCollisions(&freezeRays[i]))
		if (barriers->checkCollisions(&freezeRays[i]))
            remove(i, W_FREEZE_RAY);
		else
            i++;

	if (flame->isActive())
		 barriers->checkCollisions(flame);
}
