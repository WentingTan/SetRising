	...
	for (int y = minTiles.y - 1; y < maxTiles.y + 2; y++)
		for (int x = minTiles.x - 1; x < maxTiles.x + 2; x++)
			if (indexIsWithinBounds(x, y))
				// Hack: use enemy field to spawn weapon pickups and barriers
				if (tiles[tInfo.mapSize.x * y + x].enemy != E_NONE)
				{
					spawn(sf::Vector2i(x, y));
					//tiles[tInfo.mapSize.x * y + x].enemy = E_NONE;
				}
				//else if (tiles[tInfo.mapSize.x * y + x].enemy > E_NONE)
...
//==============================================
bool TileMap::isTileObstacle(int x, int y)
{
...
void TileMap::resetSpawnPoint(sf::Vector2i tile, int type)
{
...
void TileMap::update(float dt)
{
	...
	// Scan for enemies or weapon pickups to spawn based on the direction of scrolling in 
	// the current frame
	if (newMax.x > maxTiles.x && newMax.x + 1 < tInfo.mapSize.x)
	{
		for (int y = newMin.y; y < newMax.y; y++)
			// Hack: use enemy field to spawn weapon pickups
			if (tiles[tInfo.mapSize.x * y + newMax.x + 1].enemy != E_NONE)
				spawn(sf::Vector2i(newMax.x + 1, y));
			//else if (tiles[tInfo.mapSize.x * y + newMax.x + 1].enemy > E_NONE)
	...
	else if (newMin.x < minTiles.x && newMin.x > 0)
	{
		for (int y = newMin.y; y < newMax.y; y++)
			if (tiles[tInfo.mapSize.x * y + newMin.x - 1].enemy != E_NONE)
				spawn(sf::Vector2i(newMin.x - 1, y));
			//else if (tiles[tInfo.mapSize.x * y + newMin.x - 1].enemy > E_NONE)
	...
	if (newMax.y > maxTiles.y && newMax.y + 1 < tInfo.mapSize.y)
	{
		for (int x = newMin.x; x < newMax.x; x++)
			if (tiles[tInfo.mapSize.x * (newMax.y + 1) + x].enemy != E_NONE)
				spawn(sf::Vector2i(x, newMax.y + 1));
			//else if (tiles[tInfo.mapSize.x * (newMax.y + 1) + x].enemy > E_NONE)
	...
	else if (newMin.y < minTiles.y && newMin.y > 0)
	{
		for (int x = newMin.x; x < newMax.x; x++)
			if (tiles[tInfo.mapSize.x * (newMin.y - 1) + x].enemy != E_NONE)
				spawn(sf::Vector2i(x, newMin.y - 1));
			//else if (tiles[tInfo.mapSize.x * (newMin.y - 1) + x].enemy > E_NONE)
...
void TileMap::spawn(sf::Vector2i tile)
{
	int type = tiles[tInfo.mapSize.x * tile.y + tile.x].enemy;
...
void TileMap::spawnWeapon(sf::Vector2i tile)
{
	// Calculate the position of the tile in screen corrdinates
	sf::Vector2f pos;
	// Use the center of the tile in the x direction
	pos.x = ((float)tile.x + 0.5f) * TILE_SIZE - scrollX;
	// Use the top of the tile - tile size in the y direction
	pos.y = ((float)tile.y - 1) * TILE_SIZE - scrollY;
	
	// Generate a SPAWN_WEAPON event
	Event::Data e;
	e.type = Event::SPAWN_WEAPON;
	e.pickupType = tiles[tInfo.mapSize.x * tile.y + tile.x].enemy;
	e.posX = pos.x;
	e.posY = pos.y;
	e.tile = tile;
	EventManager::triggerEvent(e);

	// Ensure the tile doesn't continue spawning the weapon pickup
	tiles[tInfo.mapSize.x * tile.y + tile.x].enemy = E_NONE;
}

void TileMap::spawnIceblock(sf::Vector2i tile)
{
	// Calculate the position of the tile in screen corrdinates
	sf::Vector2f pos;
	// Use the center of the tile in the x direction
	pos.x = ((float)tile.x + 0.5f) * TILE_SIZE - scrollX;
	// Use the top of the tile - tile size in the y direction
	pos.y = ((float)tile.y - 0.5f) * TILE_SIZE - scrollY;
	
	// Generate a SPAWN_WEAPON event
	Event::Data e;
	e.type = Event::SPAWN_BARRIER;
	e.posX = pos.x;
	e.posY = pos.y;
	e.tile = tile;
	EventManager::triggerEvent(e);

	// Ensure the tile doesn't continue spawning the ice block
	tiles[tInfo.mapSize.x * tile.y + tile.x].enemy = E_NONE;
}
