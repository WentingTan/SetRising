...
void TileMap::enter(sf::Vector2i ref)
{
	...
	// Do an initial sweep for spawning enemies
	for (int y = minTiles.y; y < maxTiles.y + 1; y++)
		for (int x = minTiles.x; x < maxTiles.x + 1; x++)
			if (tiles[tInfo.mapSize.x * y + x].enemy == E_SOLDIER)
				spawnEnemy(sf::Vector2i(x, y), -1.0f);
}
